...
let adjustedManualWeights = 
     manualWeights 
     |> setWeightFor( "le", -20.0 )
     |> setWeightFor( "th", 20.0 )

// what does our neuron predict now?

predict adjustedManualWeights englishSample 
predict adjustedManualWeights frenchSample 
...
let distance (a :float) ( b :float ) = 
    float( a - b ) ** 2.0

let error ( trainingData : (( float * float[] )[])) ( weights : float[] ) = 
    let predictHelper x = 
        predict weights x
    trainingData
    |> Array.map( fun ( trueLabel, features ) -> 
        distance trueLabel ( predictHelper  features ))
    |> Array.sum 
