...
// returning 0 if the feature is not found or "count / total" otherwise

let countLookup = dict counts

let probabilities = 
    features 
    |> Array.map( fun x -> if countLookup.ContainsKey x then ( float countLookup.[ x ] / ( float total )) else 0.0 ) 
...
let getFeatureVector text = 
  ...
  let countLookup = dict counts 
  features |> Array.map (fun x -> if countLookup.ContainsKey x then ( float countLookup.[ x ] / ( float total )) else 0.0 ) 
...
let distance (features1:float[]) (features2:float[]) = 
    Array.map2( fun a b -> float( a - b ) ** 2.0 ) features1 features2 
    |> Array.sum
...
let classifyLanguage text =
    let featureVector = getFeatureVector text 
    let sortedDistances = 
        languageFeatures
        |> Array.map(fun (label, x) -> label, distance featureVector x ) 
        |> Array.sortBy( fun ( label, distance ) -> distance )
    Seq.head sortedDistances
